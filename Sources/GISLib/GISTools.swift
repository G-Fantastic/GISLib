...
public class GISTools {
    /// 打印当前GISLib中使用的一些参数值
    public class func printGISParams() {
        print("\n============GISLib中使用的一些参数============")
        print("          圆周率π  ➙  \(π)")
    ...
    /// 通过起始点与终点经纬度计算距离
    public class func distance(_ latLon1: GISLatLon, _ latLon2: GISLatLon) -> Double{
    ...
    public class func distanceWithAltitude(_ latLon1: GISLatLon, _ latLon2: GISLatLon, _ altitude1: Double, _ altitude2: Double) -> Double{
        let radLat1 = DEG2RAD * latLon1.latitude
        let radLat2 = DEG2RAD * latLon2.latitude
        let diffLat = radLat1 - radLat2
        let diffLon = (latLon1.longitude  - latLon2.longitude) * DEG2RAD
